...
module Isucari
  class Web < Sinatra::Base
    ...
    get '/new_items/:root_category_id.json' do
      ...
      items = if item_id > 0 && created_at > 0
        db.xquery("SELECT items.*, users.account_name, users.num_sell_items FROM `items` INNER JOIN users ON users.id = items.seller_id WHERE items.`status` IN (?, ?) AND items.category_id IN (?) AND (items.`created_at` < ?  OR (items.`created_at` <= ? AND items.`id` < ?)) ORDER BY items.`created_at` DESC, items.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", ITEM_STATUS_ON_SALE, ITEM_STATUS_SOLD_OUT, category_ids, Time.at(created_at), Time.at(created_at), item_id)
      else
        db.xquery("SELECT items.*, users.account_name, users.num_sell_items FROM `items` INNER JOIN users ON users.id = items.seller_id WHERE items.`status` IN (?,?) AND items.category_id IN (?) ORDER BY items.`created_at` DESC, items.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", ITEM_STATUS_ON_SALE, ITEM_STATUS_SOLD_OUT, category_ids)
      end
    ...
    get '/users/:user_id.json' do
      ...
      items = if item_id > 0 && created_at > 0
        # paging
        db.xquery("SELECT items.*, users.account_name, users.num_sell_items FROM items INNER JOIN users ON items.seller_id = users.id WHERE items.seller_id = ? AND items.status IN (?, ?, ?) AND items.created_at <= ? AND items.id < ? ORDER BY items.created_at DESC, items.id DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT, Time.at(created_at), item_id)
      else
        # 1st page
        db.xquery("SELECT items.*, users.account_name, users.num_sell_items FROM items INNER JOIN users ON items.seller_id = users.id WHERE items.seller_id = ? AND items.status IN (?, ?, ?) ORDER BY items.created_at DESC, items.id DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT)
      end
